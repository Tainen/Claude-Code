...
- 2026-09-16 に18本すべての Doc を新形式で作り直した（「確認前」フォルダ）。旧 Doc は同フォルダで `_old_` 接頭辞にリネーム（Drive MCP からフォルダ移動は権限エラーになるため移動していない。空フォルダ「_old（旧版の記事Doc）」が HP/お役立ち記事 直下に残っている。旧 Doc の削除・移動は堀本さんが判断）。
