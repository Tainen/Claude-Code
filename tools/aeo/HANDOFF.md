...
- **登壇回数の不一致（要確認）**：No.004 の Doc 上で「年間200回以上」が「年間100回以上」に手修正されていた（2か所）。No.004 の正本は Doc に合わせて100回にしたが、他17本・SKILL.md・本ファイルの決定事項は200回のまま。統一方針を堀本さんに確認する。
...
- 引き続き未確認：著者行の登壇回数（No.004 のみ「100回以上」、他は「200回以上」）の統一。
