...
- No.004（Doc 名は No.001_…）で装飾付き Doc を先行作成したが、**堀本さんの判断（2026-09-15）：Doc 上の装飾より Claude in Chrome でテンプレートを直す方が良い結果。** そのため Doc 生成の既定は「装飾なし・太字あり・末尾にメタ情報」に変更（`--decorate` で装飾は付けられる）。
